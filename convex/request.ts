...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Check if request already exists - note the logic was inverted
    const existingRequest = await ctx.db
      .query("requests")
      .withIndex("by_receiver_sender", (q) => q.eq("sender", currentUser._id).eq("receiver", receiver._id))
      .unique()
    ...
    // Check if request already received - note the logic was inverted
    const receivedRequest = await ctx.db
      .query("requests")
      .withIndex("by_receiver_sender", (q) => q.eq("sender", receiver._id).eq("receiver", currentUser._id))
      .unique()
